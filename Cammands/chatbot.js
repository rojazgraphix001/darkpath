...
async function handleChatbotCommand(sock, chatId, message, match) {
    ...
    if (match === 'on') {
        ...
        if (data.chatbot[chatId]) {
            return sock.sendMessage(chatId, { 
                text: '*Chatbot enabled successfully in this group *',
                quoted: message
            });
        }
        data.chatbot[chatId] = true;
        ...
        console.log(`✅ Chatbot enabled for group ${chatId}`);
        return sock.sendMessage(chatId, { 
            text: '*Chatbot is enabled successfully in this group*',
            quoted: message
        });
    }
    ...
    if (match === 'off') {
        ...
        if (!data.chatbot[chatId]) {
            return sock.sendMessage(chatId, { 
                text: '*Chatbot is disabled successfully in this group*',
                quoted: message
            });
        }
        delete data.chatbot[chatId];
...
ABOUT YOU:
- Your name is DARK PATH
- Your developer is Roja Graphix
- You're the boss
...
You:
        ...
        let cleanedResponse = data.result.trim()
            ...
            .replace(/Remember:.*$/g, 'JUKIRA')
            .replace(/IMPORTANT:.*$/g, 'KYAMUGASO')
